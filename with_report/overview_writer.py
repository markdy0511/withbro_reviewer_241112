...
def writer(rounded_overview_df, metric_col):
    ...
    if len(rounded_overview_df) == 1:
        sentences = {"데이터 없음":"비교할 수 있는 기간이 없습니다."}
        descript_raw = {"데이터 없음":"비교할 수 있는 기간이 없습니다."}
        ...
        for col in columns:
            ...
            if change_period[col] != 0 and change_period[col] != None and not(pd.isna(change_period[col])):
                if col in ["총비용", "구매액", "CPC", "CPA", "GA_CPA", "객단가", "GA_객단가"]:
                    ...
                    sentences[col] = f"{col}은 지난 기간 대비 {abs(change_rate_period[col]):,.2f} % {change_rate}하였습니다. ({previous_period[col]:,.2f} % -> {current_period[col]:,.2f} %, {change_rate_period[col]:,.2f} %)"
                else:
                    sentences[col] = f"{col}은 지난 기간 대비 {abs(change_period[col]):,.0f} 건 {change}하였습니다. ({previous_period[col]:,.0f} 건 -> {current_period[col]:,.0f} 건, {change_rate_period[col]:,.2f} %)"
            elif change_period[col] == None or pd.isna(change_period[col]):
                sentences[col] = f"{col} 데이터가 없습니다."
            else:
                sentences[col] = f"{col}은 지난 기간과 동일합니다."
    ...
    if "구매" not in media_trans and "GA_구매" not in ga_trans:
        if current_period["전환수"] != 0 and current_period["전환수"] != None and not(pd.isna(current_period["전환수"])):
            ...
            descript_raw["전환 성과"] = "전환된 성과가 없습니다."
        else:
            descript_raw["전환 성과"] = "분석할 전환 데이터가 없습니다."
        if current_period["GA_전환수"] != 0 and current_period["GA_전환수"] != None and not(pd.isna(current_period["GA_전환수"])):
            ...
            descript_raw["GA 전환 성과"] = "전환된 GA 성과가 없습니다."
        else:
            descript_raw["GA 전환 성과"] = "분석할 GA 전환 데이터가 없습니다."
    else:
        if current_period["구매"] != 0 and current_period["구매"] != None and not(pd.isna(current_period["구매"])):
            ...
            descript_raw["전환 성과"] = "구매 성과가 없습니다."
        else:
            descript_raw["전환 성과"] = "분석할 구매 데이터가 없습니다."
        if current_period["GA_구매"] != 0 and current_period["GA_구매"] != None and not(pd.isna(current_period["GA_구매"])):
            ...
            descript_raw["GA 전환 성과"] = "GA 구매 성과가 없습니다."
        else:
            descript_raw["GA 전환 성과"] = "분석할 GA 구매 데이터가 없습니다."
